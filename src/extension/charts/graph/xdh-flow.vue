    ...
    optionsSelf() {
      ...
      d.forEach((item, index) => {
        ...
        item.forEach((subItem, subIndex) => {
          // 当前标点
          let currentCoords = Array.isArray(subItem.value)
            ? subItem.value.join('_')
            : subItem.value;
          ...
          if (subIndex > 0) {
            // 连线
            let oldName =
              item[subIndex - 1].name ||
              (Array.isArray(item[subIndex - 1].value)
                ? item[subIndex - 1].value.join('_')
                : item[subIndex - 1].value);
      ...
      return (echart, vm) => {
        if (this.$attrs.options) {
          _options = Object.assign(
            {},
            _options,
            (typeof this.$attrs.options === 'function'
              ? this.$attrs.options(echart, vm)
              : this.$attrs.options) || {}
          );
        }
        return _options;
